...
var adapter = function(application){
    ...
    if( !this.app._memcachestore )
        this.app._memcachestore = this.app.serviceManager.get('memcached');
    this.app._memcachestore.set('toto', {aaa:'je suis en cache',bbb:125}, 10, function(err,datas){
        console.log(err,datas);
    });
    this.app._memcachestore.get('toto', function(err,datas){
        console.log(err,datas, typeof(datas));
    });
};
...
/*adapter.prototype.exist = function( key, next ){
    next( undefined, this.app._memcachestore[key]?true:false );
};
